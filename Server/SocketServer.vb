...
Public Class SocketServer
    ...
    ''' <summary>
    ''' Initializes a new SocketServer listening on the specified port and listening on all available IP addresses
    ''' </summary>
    ...
    ''' <summary>
    ''' Initializes a new SocketServer listening on the specified IP address and port only
    ''' </summary>
    ''' <param name="Address">IP address to bind to on the local machine</param>
    ''' <param name="Port">TCP port to listen on (1-65535)</param>
    ''' <remarks></remarks>
    ...
    Sub ClientAccepted(ByVal ar As IAsyncResult)
        ...
        Catch ex As ObjectDisposedException
            'HACK if we get an ObjectDisposedException it that means the server socket terminated while this async method was still active
            Exit Sub
        End Try
    ...
    Sub ClientMessageReceived(ByVal ar As IAsyncResult)
        ...
        If numBytesReceived = 0 Then
            ClientList.Remove(mState.Socket)
            RaiseEvent ClientDisconnected(mState.Socket)
            Exit Sub
        End If
